...
<div class="wrap">
    ...
    <!-- Default Package Type Setting -->
    <div class="wsl-default-package-type-setting">
        <table class="form-table">
            <tr valign="top">
                <th scope="row"><?php _e('Default Package Type', 'woo-shipping-labels'); ?></th>
                <td>
                    <select name="wsl_default_package_type">
                        <option value="small_box"><?php _e('Small Box', 'woo-shipping-labels'); ?></option>
                        <option value="large_box"><?php _e('Large Box', 'woo-shipping-labels'); ?></option>
                        <option value="custom"><?php _e('Custom Package', 'woo-shipping-labels'); ?></option>
                    </select>
                    <p class="description">
                        <?php _e('Choose the default package type for label creation.', 'woo-shipping-labels'); ?>
                    </p>
                </td>
            </tr>
        </table>
                ...
                <tbody>
                    ...
                    <?php foreach ($carrier_packages as $carrier => $packages) : ?>
                        <?php foreach ($packages as $id => $package) : ?>
                            <tr>
                                <td>
                                    <input type="checkbox" name="enabled_packages[]" value="<?php echo esc_attr($carrier . '_' . $id); ?>" <?php checked(in_array($carrier . '_' . $id, $enabled_packages)); ?>>
                                </td>
                                <td><?php echo esc_html($package['name']); ?></td>
                                <td><?php echo esc_html($package['weight']); ?></td>
                                <td><?php echo esc_html($package['length'] . ' x ' . $package['width'] . ' x ' . $package['height']); ?></td>
